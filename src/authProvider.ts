...
import { AuthApiFactory, Configuration } from "./services";

const authProvider: AuthProvider = {
  login: async ({ username, password }) => {
    const {
      data: { access_token },
    } = await AuthApiFactory().authorize({ username, password });
  ...
  getIdentity: async () => {
    ...
    const { data } = await AuthApiFactory(configuration).profile();
